...
```{r, fig.alt = "RStudio layout with three default panes"}
knitr::include_graphics("fig/01-rstudio.png")
```
...
```{r, fig.alt = "RStudio 4-pane layout with .R file open"}
knitr::include_graphics("fig/01-rstudio-script.png")
```
...
```{r, fig.alt = "Image of a local folder structure with files. The file names do not easily make it possible to understand which files are similar in content or which is the newest version."}
knitr::include_graphics("fig/01_bad_project.png")
```
